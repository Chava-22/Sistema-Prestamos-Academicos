...
ARCHIVO_DATOS = Path(__file__).resolve().parent / "datos_catalogo.json"
...
def guardar_datos():
    datos = [producto.model_dump() for producto in catalogo.listar_productos()]
...
def cargar_datos():
    ...
    for item in datos:
        try:
            catalogo.agregar_producto(Equipo(**item))
...
def main(page: ft.Page):
    ...
    page.add(
        ...
        ft.Container(
            content=ft.Column(
                controls=[fila_stats, tarjeta_formulario, tarjeta_tabla],
                spacing=20,
            ),
            padding=ft.padding.symmetric(horizontal=32, vertical=26),
        ),
